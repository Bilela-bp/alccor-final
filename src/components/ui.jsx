...
function Pagination({ page, totalPages, totalItems, pageSize = 12, onPageChange }) {
  if (totalItems <= pageSize) return null;
  return (
    <div className="flex items-center justify-between border-t border-stone-200 px-4 py-3">
      <span className="text-xs text-stone-500">Página {page} de {totalPages} · {totalItems} registro(s)</span>
      <div className="flex items-center gap-2">
        <button type="button" disabled={page === 1} onClick={() => onPageChange(Math.max(1, page - 1))} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-sm border border-stone-200 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed hover:bg-stone-50"><ChevronLeft size={15}/> Anterior</button>
        <button type="button" disabled={page === totalPages} onClick={() => onPageChange(Math.min(totalPages, page + 1))} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-sm border border-stone-200 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed hover:bg-stone-50">Próxima <ChevronRight size={15}/></button>
      </div>
    </div>
  );
}
