...
function Stat({ label, value, tone = "default", sub }) {
  ...
  return (
    <div
      className="bg-white border border-stone-200 rounded-xl p-4 flex-1"
      style={{ minWidth: 160 }}
    >
...
function Modal({ title, onClose, children, wide }) {
  return (
    <div
      ...
      style={{ background: "rgba(36,24,15,0.45)" }}
    >
      <div
...
const inputCls =
  "w-full border border-stone-300 rounded-lg px-3 py-2 text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-amber-700/40 focus:border-amber-700";
    ...
    <div className="flex items-center justify-between border-t border-stone-200 px-4 py-3">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-1 px-2.5 py-1.5 text-sm border border-stone-200 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed hover:bg-stone-50"
        >
      ...
      {open && (
        ...
        >
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="absolute top-5 right-5 rounded-lg p-2 text-white hover:bg-white/15"
        ...
        <div className="absolute z-30 mt-1 w-full max-h-60 overflow-y-auto bg-white border border-stone-200 rounded-lg shadow-lg">
          {allowClear && value && (
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-stone-400 hover:bg-stone-50 border-b border-stone-100"
            >
              Limpar seleção
            </button>
          )}
...
function PrimaryButton({ children, onClick, type = "button", disabled, full }) {
  return (
    <button
      ...
      onMouseEnter={(e) =>
        !disabled && (e.currentTarget.style.backgroundColor = WOOD.accentDark)
      }
      onMouseLeave={(e) =>
        !disabled && (e.currentTarget.style.backgroundColor = WOOD.accent)
      }
    >
      {children}
    </button>
...
function LoadingRows() {
  return (
    <div className="py-12 text-center text-sm text-stone-400">Carregando…</div>
  );
}
    ...
    <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
      <div className="flex items-center gap-3">
        <div
          ...
          style={{ backgroundColor: "#E7E5E3", color: WOOD.accentDark }}
        >
          <Icon size={20} />
        </div>
        <div>
