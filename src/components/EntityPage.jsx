...
export default function EntityPage({
  ...
  const refFields = useMemo(
    () => fields.filter((f) => f.refTable || f.type === "grupo"),
    [fields],
  );
  ...
  async function handleDelete(row) {
    if (
      !window.confirm("Excluir este registro? Essa ação não pode ser desfeita.")
    )
      return;
    try {
      ...
      const isForeignKeyError =
        /foreign key constraint|violates foreign key/i.test(e.message);
    ...
    try {
      ...
      setLookups((prev) => {
        ...
        const jaExiste = atual.some(
          (o) => o.label.toLowerCase() === nome.toLowerCase(),
        );
        const novaLista = jaExiste
          ? atual
          : [...atual, { id: nome, label: nome }].sort((a, b) =>
  ...
  async function handleSave(e) {
    ...
    for (const f of fields) {
      ...
      if (f.type === "cnpj") {
        ...
        if (f.required && digits.length === 0) {
          window.alert(`Informe o CNPJ em "${f.label}".`);
          return;
        }
        if (digits.length > 0 && !isValidCNPJ(v)) {
          window.alert("CNPJ inválido. Confira os números antes de salvar.");
          return;
        }
      }
      if (f.type === "email" && v && !isValidEmail(v)) {
        window.alert(
          `E-mail inválido em "${f.label}". Confira se digitou o @ e o domínio corretamente (ex: nome@empresa.com).`,
        );
        return;
      }
      if (
      ...
      if (f.type === "documento") {
        ...
        if (digits.length === 0) {
          if (f.required) {
            window.alert("Informe o CPF ou CNPJ do cliente.");
            return;
          }
        } else {
          ...
          if (!valido) {
            window.alert(
              `${tipo.toUpperCase()} inválido. Confira os números antes de salvar.`,
            );
            return;
          }
        }
    ...
    try {
      ...
      fields.forEach((f) => {
        ...
        payload[f.key] = v;
        if (f.type === "documento")
          payload[f.tipoKey] = formData[f.tipoKey] || "cpf";
      });
      const editingBefore = editing;
      const result = editingBefore
        ? await updateRow(table, editingBefore.id, payload)
              ...
              <thead>
                <tr className="border-b border-stone-200 bg-stone-50">
                  {listFields.map((f) => (
                    <th
                      key={f.key}
                      className="text-left px-4 py-2.5 font-medium text-stone-500 break-words"
                    >
                      {f.label}
                    </th>
                  ))}
                ...
                {paginatedRows.map((row) => (
                  ...
                  >
                    {listFields.map((f) => (
                      <td
                        key={f.key}
                        className="px-4 py-2.5 text-stone-800 align-top whitespace-normal break-words max-w-[220px]"
                      >
                        {f.render ? (
                          ...
                          fmtDate(row[f.key])
                        ) : f.type === "number" && f.currency ? (
                          fmtCurrency(row[f.key])
                        ) : (
                          (row[f.key] ?? "—")
                        )}
                      </td>
                ...
                >
                  {f.type === "select" && f.options ? (
                    ...
                    <ComboSelect
                      ...
                      value={formData[f.key] || ""}
                      onChange={(nome) =>
                        setFormData({ ...formData, [f.key]: nome })
                      }
                    ...
                    <>
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            [f.key]: maskCPF(e.target.value),
                          })
                        }
                      />
                      {formData[f.key] &&
                        formData[f.key].replace(/\D/g, "").length === 11 &&
                        !isValidCPF(formData[f.key]) && (
                    ...
                    <>
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            [f.key]: maskCNPJ(e.target.value),
                          })
                        }
                      />
                      {formData[f.key] &&
                        formData[f.key].replace(/\D/g, "").length === 14 &&
                        !isValidCNPJ(formData[f.key]) && (
                          <span className="text-xs text-red-600 mt-1 block">
                    ...
                    <>
                      ...
                      {formData[f.key] && !isValidEmail(formData[f.key]) && (
                        <span className="text-xs text-red-600 mt-1 block">
                          E-mail inválido — precisa ter @ e um domínio (ex:
                          nome@empresa.com).
                        </span>
                      )}
                    </>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          [f.key]: maskPhone(e.target.value),
                        })
                      }
                    />
                  ) : f.type === "documento" ? (
                    <>
                      <div className="flex gap-2 mb-2">
                        {["cpf", "cnpj"].map((tipo) => (
                          <button
                            ...
                            style={
                              (formData[f.tipoKey] || "cpf") === tipo
                                ? {
                                    backgroundColor: WOOD.accent,
                                    color: "#fff",
                                    borderColor: WOOD.accent,
                                  }
                      ...
                      {(() => {
                        const tipo = formData[f.tipoKey] || "cpf";
                        const digits = (formData[f.key] || "").replace(
                          /\D/g,
                          "",
                        );
                        const complete =
                        ...
                        return complete && !valido ? (
                          <span className="text-xs text-red-600 mt-1 block">
                            {tipo.toUpperCase()} inválido — confira os números.
                          </span>
                        ) : null;
                      })()}
                    </>
        ...
        <Modal title="Criar novo grupo" onClose={() => setNovoGrupoFor(null)}>
          ...
          >
            ...
            <div className="flex gap-2 justify-end">
              <SecondaryButton onClick={() => setNovoGrupoFor(null)}>
                Cancelar
              </SecondaryButton>
