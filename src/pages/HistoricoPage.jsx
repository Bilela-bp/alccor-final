...
export default function HistoricoPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      window.alert("Erro ao carregar histórico: " + e.message);
    }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const userName = (id) =>
    usuarios.find((u) => u.id === id)?.nome || "Usuário removido";
  const tableLabel = (t) =>
    ({
      ...
      notas_fiscais_itens: "Itens de notas",
  ...
  const filtered = rows.filter((r) => {
    ...
    if (filtroData) {
      const diaLocal = new Date(r.created_at).toLocaleDateString("en-CA", {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Quem alterou, o que foi alterado e quando"
      >
      ...
      <div className="bg-white border border-stone-200 rounded-xl p-4 mb-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <Field label="Tabela / módulo">
          ...
          >
            <option value="">Todos</option>
            {[...new Set(rows.map((r) => r.tabela))].map((t) => (
      ...
      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={History} text="Nenhuma alteração encontrada." />
        ) : (
          <>
              ...
              <table className="w-full text-sm table-fixed">
                <thead>
                  <tr className="bg-stone-50 border-b border-stone-200">
                    <th className="text-left px-4 py-2.5 w-44">Data</th>
                    <th className="text-left px-4 py-2.5 w-48">Usuário</th>
                    <th className="text-left px-4 py-2.5 w-52">
                      Onde foi feito
                    </th>
                    <th className="text-left px-4 py-2.5">O que foi feito</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.map((r) => (
