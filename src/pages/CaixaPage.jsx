...
export default function CaixaPage({ user }) {
  ...
  async function abrirCaixa(e) {
    ...
    try {
      ...
      window.alert("Erro ao abrir caixa: " + e.message);
    }
  ...
  async function openDetail(caixa) {
    const data = await get(
      "movimentacoes_caixa",
      `&caixa_id=eq.${caixa.id}&order=data.desc`,
    );
    setMovs(data || []);
    setDetail(caixa);
  }
  ...
  async function addMov(e) {
    ...
    if (!movForm.valor || Number(movForm.valor) <= 0) {
    ...
    try {
      ...
      const data = await get(
        "movimentacoes_caixa",
        `&caixa_id=eq.${detail.id}&order=data.desc`,
      );
      setMovs(data || []);
    } catch (e) {
      window.alert("Erro ao lançar movimentação: " + e.message);
    }
  ...
  async function fecharCaixa() {
    ...
    if (
      !window.confirm(
        `Fechar caixa com saldo de ${fmtCurrency(valorFechamento)}?`,
      )
    )
      return;
    fecharRef.current = true;
    try {
      await updateRow("caixa", detail.id, {
        status: "fechado",
      ...
      window.alert("Erro ao fechar caixa: " + e.message);
    }
  ...
  const paginatedCaixas = caixas.slice(
  ...
  return (
    <div>
              ...
              {paginatedCaixas.map((c) => (
                ...
                >
                  <td className="px-4 py-2.5 text-stone-800">
                    {fmtDateTime(c.data_abertura)}
                  </td>
                  <td className="px-4 py-2.5 text-stone-700">
                    {fmtCurrency(c.valor_abertura)}
                  </td>
                  <td className="px-4 py-2.5 text-stone-700">
                  ...
                  <td className="px-4 py-2.5 text-stone-700">
                    {c.valor_fechamento != null
                      ? fmtCurrency(c.valor_fechamento)
                      : "—"}
                  </td>
                  <td className="px-4 py-2.5">
                    <Badge tone={c.status === "aberto" ? "green" : "gray"}>
                      {c.status}
                    </Badge>
                  </td>
      ...
      {detail && (
        <Modal
          title={`Caixa de ${fmtDateTime(detail.data_abertura)}`}
          onClose={() => setDetail(null)}
          wide
        >
          <div className="flex items-center justify-between mb-4">
            <Badge tone={detail.status === "aberto" ? "green" : "gray"}>
              {detail.status}
            </Badge>
            {detail.status === "aberto" && (
              <div className="flex gap-2">
                <SecondaryButton onClick={() => setMovModal(true)}>
                  <Plus size={14} /> Movimentação
                </SecondaryButton>
                <PrimaryButton onClick={fecharCaixa}>
          ...
          {movs.length === 0 ? (
            ...
            <div className="space-y-1">
              {movs.map((m) => (
                <div
                  key={m.id}
                  className="flex items-center justify-between text-sm border-b border-stone-100 py-2"
                >
                  <div>
                    ...
                    <p className="text-xs text-stone-400">
                      {fmtDateTime(m.data)} · {m.forma_pagamento}
                      {nomeCliente(m.cliente_id)
                        ? ` · ${nomeCliente(m.cliente_id)}`
                        : ""}
                      {nomeFornecedor(m.fornecedor_id)
                        ? ` · ${nomeFornecedor(m.fornecedor_id)}`
                        : ""}
                    </p>
                  </div>
                  <span
                    className={`font-medium ${m.tipo === "entrada" ? "text-green-700" : "text-red-600"}`}
                  >
          ...
          >
            ...
            <Field label="Valor" required>
              <input
                ...
                value={movForm.valor}
                onChange={(e) =>
                  setMovForm({ ...movForm, valor: e.target.value })
                }
              />
            </Field>
            <Field label="Forma de pagamento">
              <select
                className={inputCls}
                value={movForm.forma_pagamento}
                onChange={(e) =>
                  setMovForm({ ...movForm, forma_pagamento: e.target.value })
                }
              >
            ...
            <Field label="Categoria">
              <input
                ...
                onChange={(e) =>
                  setMovForm({ ...movForm, categoria: e.target.value })
                }
                placeholder="Ex: Venda, Despesa, Frete…"
              />
            </Field>
            <Field label="Descrição">
              <input
                ...
                onChange={(e) =>
                  setMovForm({ ...movForm, descricao: e.target.value })
                }
              />
            </Field>
            ...
            {movForm.tipo === "saida" && (
              <Field label="Fornecedor (opcional)">
                <ComboSelect
                  options={fornecedores.map((c) => ({
                    id: c.id,
                    label: c.nome,
                  }))}
                  value={movForm.fornecedor_id}
                  onChange={(id) =>
                    setMovForm({ ...movForm, fornecedor_id: id })
                  }
            ...
            <div className="flex gap-2 justify-end mt-2">
              <SecondaryButton onClick={() => setMovModal(false)}>
                Cancelar
              </SecondaryButton>
