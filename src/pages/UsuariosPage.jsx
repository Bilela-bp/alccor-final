...
export default function UsuariosPage() {
  ...
  async function handleCreate(e) {
    ...
    if (!isValidEmail(newForm.email)) {
      window.alert(
        "E-mail inválido. Confira se digitou o @ e o domínio corretamente (ex: nome@empresa.com).",
      );
      return;
    }
    if (newForm.senha.length < 6) {
      window.alert("A senha precisa ter pelo menos 6 caracteres.");
      return;
    }
    if (newForm.senha !== newForm.confirmarSenha) {
    ...
    try {
      ...
      await load();
      window.alert(
        `Usuário criado! Repasse a senha combinada para ${newForm.nome} entrar no sistema. Se o Supabase estiver com confirmação de e-mail ativada, a pessoa vai precisar confirmar o e-mail antes do primeiro login (confira em Authentication → Providers → Email).`,
      );
    } catch (e) {
  ...
  async function handleDeactivate(row) {
    if (
      !window.confirm(
        `Desativar o acesso de "${row.nome}"? A pessoa não vai mais conseguir entrar no sistema, mas o histórico dela é mantido.`,
      )
    )
      return;
    try {
            ...
            <tbody>
              {paginatedUsuarios.map((u) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-stone-700 capitalize">
                    {u.cargo}
                  </td>
          ...
          >
            <Field label="Nome" required>
              <input
                ...
                value={newForm.nome}
                onChange={(e) =>
                  setNewForm({ ...newForm, nome: e.target.value })
                }
              />
            </Field>
            ...
            <Field label="Cargo" required>
              <select
                ...
                value={newForm.cargo}
                onChange={(e) =>
                  setNewForm({ ...newForm, cargo: e.target.value })
                }
              >
                {CARGOS_USUARIO.map((c) => (
            ...
            <Field label="Senha provisória" required>
              <input
                ...
                value={newForm.senha}
                onChange={(e) =>
                  setNewForm({ ...newForm, senha: e.target.value })
                }
              />
            </Field>
            <Field label="Confirmar senha" required>
              <input
                ...
                required
                minLength={6}
                value={newForm.confirmarSenha}
                onChange={(e) =>
                  setNewForm({ ...newForm, confirmarSenha: e.target.value })
                }
              />
            </Field>
            <p className="text-xs text-stone-400 mb-4">
              Combine essa senha com a pessoa — ela pode trocar depois pelo
              próprio Supabase, se você quiser habilitar isso futuramente.
            </p>
            <div className="flex gap-2 justify-end">
              <SecondaryButton onClick={() => setNewModal(false)}>
                Cancelar
              </SecondaryButton>
          ...
          >
            ...
            <Field label="Ativo">
              <select
                ...
                onChange={(e) =>
                  setEditForm({ ...editForm, ativo: e.target.value === "true" })
                }
              >
            ...
            <p className="text-xs text-stone-400 mb-4">
              E-mail e senha não são editáveis por aqui. Para redefinir a senha
              de alguém, use Authentication → Users → ... → Reset password no
              painel do Supabase.
            </p>
            <div className="flex gap-2 justify-end">
