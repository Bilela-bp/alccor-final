...
export default function RelatoriosPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      window.alert("Erro ao carregar relatórios: " + e.message);
    }
    setLoading(false);
  }, []);
  ...
  function exportarCSV() {
    ...
    if (tab === "receber")
      ...
      data = cp.map((r) => [
        r.descricao,
        nomeFornecedor(r.fornecedor_id),
        r.data_vencimento,
        r.valor,
        r.status,
      ]);
    ...
    a.download = `relatorio-${tab}-${inicio}-${fim}.csv`;
    a.click();
    ...
    <div>
      ...
      >
        <div className="flex gap-2">
          ...
          <SecondaryButton onClick={exportarCSV}>Exportar CSV</SecondaryButton>
        </div>
      </PageHeader>
      ...
      {tab === "receber" && (
        <ReportTable
          ...
          statusLabels={{ pendente: "Pendente", recebido: "Recebido" }}
          total={total(cr)}
        />
      )}
      {tab === "pagar" && (
      ...
      {tab === "fluxo" && (
        <div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            <Stat
              label="Entradas"
              value={fmtCurrency(total(entradas))}
              tone="green"
              sub={`${entradas.length} movimentação(ões)`}
            />
            <Stat
              label="Saídas"
              value={fmtCurrency(total(saidas))}
              tone="red"
              sub={`${saidas.length} movimentação(ões)`}
            />
            <Stat
              label="Saldo do período"
              value={fmtCurrency(total(entradas) - total(saidas))}
              tone={total(entradas) - total(saidas) >= 0 ? "green" : "red"}
              sub={`${inicio} a ${fim}`}
            />
          </div>
...
function ReportTable({ title, rows, nameFor, dateKey, statusLabels, total }) {
  return (
    <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
      ...
      {rows.length === 0 ? (
        <EmptyState icon={FileText} text="Nenhum lançamento no período." />
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-stone-50 border-b border-stone-200">
                <th className="text-left px-4 py-2.5">Descrição</th>
                <th className="text-left px-4 py-2.5">Favorecido</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-stone-100">
                  ...
                  <td className="px-4 py-2.5">{fmtDate(r[dateKey])}</td>
                  <td className="px-4 py-2.5 text-right">
                    {fmtCurrency(r.valor)}
                  </td>
                  <td className="px-4 py-2.5">
                    {r.status === "recebido" || r.status === "pago" ? (
                      <Badge tone="green">{statusLabels[r.status]}</Badge>
                    ) : (
                      <Badge tone="amber">
