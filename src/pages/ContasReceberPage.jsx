...
export default function ContasReceberPage({ user }) {
  ...
  async function handleSave(e) {
    ...
    const intervalo = Math.max(
      1,
      Math.floor(Number(form.intervalo_dias) || 30),
    );
    ...
    try {
      if (editing) {
        ...
        await registrarRecebimentoNoCaixa(
        ...
        const rowsToInsert = Array.from({ length: parcelas }, (_, i) => {
          ...
          return {
            descricao:
              parcelas > 1
                ? `${form.descricao.trim()} - Parcela ${i + 1}/${parcelas}`
                : form.descricao.trim(),
            cliente_id: form.cliente_id || null,
            valor: valorParcela,
  ...
  const totalVencido = rows
    .filter((r) => r.status === "pendente" && r.data_vencimento < hoje)
    .reduce((s, r) => s + Number(r.valor || 0), 0);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Controle de recebimentos e parcelamentos automáticos"
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <Stat
          ...
          tone="amber"
          sub={`${rows.filter((r) => r.status === "pendente").length} lançamento(s)`}
        />
        <Stat
          label="Vencido"
          value={fmtCurrency(totalVencido)}
          tone="red"
          sub="Pendências em atraso"
        />
        <Stat
          label="Recebido"
          value={fmtCurrency(
      ...
      <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={ArrowDownCircle} text="Nenhuma conta encontrada." />
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {paginated.map((r) => (
                  ...
                  >
                    <td className="px-4 py-2.5 font-medium text-stone-800 align-top whitespace-normal break-words max-w-[220px]">
                      {r.descricao}
                    </td>
          ...
          <form onSubmit={handleSave}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {!editing && (
                <Field
                  ...
                  hint="O sistema cria todas as parcelas automaticamente."
                >
                  <input
                    type="number"
                    min="1"
              ...
              {!editing && (
                <Field label="Intervalo entre parcelas (dias)">
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, intervalo_dias: e.target.value })
                    }
                  />
                </Field>
              )}
            ...
            {!editing && Number(form.parcelas) > 1 && (
              <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-700">
                Serão criadas <strong>{form.parcelas}</strong> parcelas de
                aproximadamente{" "}
                <strong>
                  {fmtCurrency(
                ...
                , com vencimentos a cada{" "}
                <strong>{form.intervalo_dias} dias</strong>.
              </div>
            )}
