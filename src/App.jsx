...
export default function App() {
      ...
      if (getCurrentSession()?.access_token) {
        try {
          const rows = await get(
            "usuarios",
            `&id=eq.${getCurrentSession().user.id}`,
          );
  ...
  const pageContent = () => {
    switch (page) {
      ...
      case "usuarios":
        return user.cargo === "admin" ? <UsuariosPage /> : <AcessoRestrito />;
      case "historico":
        return user.cargo === "admin" ? <HistoricoPage /> : <AcessoRestrito />;
      default:
        return null;
    }
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <span className="font-display text-white font-semibold text-sm leading-tight">
            ALCCOR
            <br />
            <span className="text-stone-400 font-normal text-xs font-body">
              movelaria — sistema de gestão
            </span>
          </span>
        </div>
        <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
          {NAV.filter((item) => !item.adminOnly || user.cargo === "admin").map(
            (item) => (
              <button
                key={item.key}
                ...
                style={
                  page === item.key
                    ? { backgroundColor: WOOD.accent, color: "#fff" }
                    : { color: "#ADB2B7" }
                }
                onMouseEnter={(e) => {
                  if (page !== item.key)
                    e.currentTarget.style.backgroundColor = WOOD.sidebarBorder;
                }}
                onMouseLeave={(e) => {
                  if (page !== item.key)
        ...
        >
          <div className="min-w-0">
            ...
            <p className="text-xs truncate" style={{ color: "#8B9198" }}>
              {user.cargo}
            </p>
          </div>
          <button
      ...
      <main className="flex-1 p-6 overflow-y-auto">{pageContent()}</main>
    </div>
  );
}
