...
import { getCurrentSession, signIn, signUpUser, signOut, get, insertRow, insertRows, updateRow, deleteRow } from './lib/supabase';
import { fmtCurrency, fmtDate, fmtDateTime, todayISO, monthISO, maskCPF, isValidCPF, maskCNPJ, isValidCNPJ, maskPhone, isValidEmail } from './lib/helpers';
import { WOOD } from './lib/theme';
import { FontStyles, Badge, Stat, Modal, Field, ComboSelect, PrimaryButton, SecondaryButton, EmptyState, LoadingRows, ImageLightbox, inputCls } from './components/ui';
...
function Dashboard({ user }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [produtos, contasPagar, contasReceber, caixaAberto, movimentacoes] = await Promise.all([
      ...
      setData({ produtos, contasPagar, contasReceber, caixaAberto, movimentacoes });
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
  ...
  const ultimosDias = Array.from({ length: 7 }, (_, i) => {
    ...
    return { iso, label: d.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }) };
  });
  const fluxoGrafico = ultimosDias.map(d => {
    const doDia = data.movimentacoes.filter(m => String(m.data || '').slice(0, 10) === d.iso);
    return { ...d, entradas: doDia.filter(m => m.tipo === 'entrada').reduce((s, m) => s + Number(m.valor || 0), 0), saidas: doDia.filter(m => m.tipo === 'saida').reduce((s, m) => s + Number(m.valor || 0), 0) };
  });
  const maxFluxo = Math.max(1, ...fluxoGrafico.flatMap(d => [d.entradas, d.saidas]));

  const receberGrafico = ultimosDias.map(d => ({
    ...d,
    total: data.contasReceber.filter(c => String(c.data_vencimento || '').slice(0, 10) === d.iso).reduce((s, c) => s + Number(c.valor || 0), 0)
  }));
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-5">
        <div className="bg-white border border-stone-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-medium text-stone-800 text-sm">Fluxo de caixa</h3><p className="text-xs text-stone-400 mt-0.5">Últimos 7 dias</p></div>
            <div className="flex gap-3 text-xs text-stone-500"><span className="flex items-center gap-1"><i className="w-2 h-2 rounded-full bg-green-500"/>Entradas</span><span className="flex items-center gap-1"><i className="w-2 h-2 rounded-full bg-red-500"/>Saídas</span></div>
          </div>
          <div className="h-44 flex items-end gap-2 border-b border-stone-200">
            {fluxoGrafico.map(d => (
              <div key={d.iso} className="flex-1 h-full flex items-end justify-center gap-1 group" title={`${d.label} — Entradas ${fmtCurrency(d.entradas)} | Saídas ${fmtCurrency(d.saidas)}`}>
                <div className="w-3 rounded-t bg-green-500/80" style={{ height: `${Math.max(3, d.entradas / maxFluxo * 100)}%` }} />
                <div className="w-3 rounded-t bg-red-500/75" style={{ height: `${Math.max(3, d.saidas / maxFluxo * 100)}%` }} />
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-2 mt-2 text-[10px] text-stone-400 text-center">{fluxoGrafico.map(d => <span key={d.iso}>{d.label}</span>)}</div>
        </div>

        <div className="bg-white border border-stone-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-4"><div><h3 className="font-medium text-stone-800 text-sm">Contas a receber</h3><p className="text-xs text-stone-400 mt-0.5">Vencimentos nos últimos 7 dias</p></div><ArrowDownCircle size={18} className="text-stone-400"/></div>
          <div className="h-44 flex items-end gap-2 border-b border-stone-200">
            {receberGrafico.map(d => (
              <div key={d.iso} className="flex-1 h-full flex items-end justify-center" title={`${d.label} — ${fmtCurrency(d.total)}`}>
                <div className="w-full max-w-8 rounded-t bg-stone-500/80" style={{ height: `${Math.max(3, d.total / maxReceber * 100)}%` }} />
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-2 mt-2 text-[10px] text-stone-400 text-center">{receberGrafico.map(d => <span key={d.iso}>{d.label}</span>)}</div>
        </div>
      </div>
  ...
  return <div>
    ...
    <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-stone-50 border-b border-stone-200">
        <th className="text-left px-4 py-2.5">Data</th><th className="text-left px-4 py-2.5">Usuário</th><th className="text-left px-4 py-2.5">Onde foi feito</th><th className="text-left px-4 py-2.5">O que foi feito</th>
      </tr></thead><tbody>{filtered.map(r=><tr key={r.id} className="border-b border-stone-100 align-top">
        <td className="px-4 py-2.5 whitespace-nowrap">{fmtDateTime(r.created_at)}</td><td className="px-4 py-2.5">{userName(r.usuario_id)}</td><td className="px-4 py-2.5">{tableLabel(r.tabela)}</td><td className="px-4 py-2.5"><Badge tone={r.acao==='DELETE'?'red':r.acao==='INSERT'?'green':'amber'}>{resumo(r)}</Badge></td>
      </tr>)}</tbody></table></div>}
    </div>
  </div>;
}
