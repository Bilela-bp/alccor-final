...
A estrutura foi organizada para separar responsabilidades sem alterar o comportamento do sistema:

- `src/App.jsx` — composição das telas e regras específicas das páginas.
- `src/components/ui.jsx` — componentes visuais reutilizáveis (botões, modal, campos, badges e estados).
- `src/lib/supabase.js` — autenticação, sessão e chamadas REST do Supabase.
- `src/lib/helpers.js` — máscaras, validações, datas e formatação monetária.
- `src/lib/theme.js` — cores e constantes visuais do sistema.
- `src/assets/` — imagens e arquivos estáticos.
- `database/migrations/` — scripts de criação/alteração do banco.
- `database/diagnostics/` — scripts de diagnóstico e verificação.

A pasta duplicada `alccor-sistema/` foi removida do pacote final.
